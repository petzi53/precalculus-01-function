...
To learn and remember better which R packages are used I am not using the `library()` function but will add the package name before the function name like `ggplot2::aes()`. Exceptions are those function that are part of one of the packages of R standard system library.
...
```{r}
quad_func <- tibble::tibble(x = seq(-10, 10, 0.01),
                    y = -(1 / 7) * (x^2 + 7),
                    y2 = -(1 / 7) * (x - 7) * (x + 7),
                    y3 = -(1 / 7) * (x^2 - 49))

ggplot2::ggplot(quad_func, ggplot2::aes(x, y2)) +
  ggplot2::geom_line(breaks = 1:10) +
